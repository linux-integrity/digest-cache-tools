 ...
 */
static int digest_list_parse_rpm(struct digest_cache *digest_cache,
...
static struct parser rpm_parser = {
	.name = "rpm",
	.owner = THIS_MODULE,
	.func = digest_list_parse_rpm,
};

static int __init rpm_parser_init(void)
{
	return digest_cache_register_parser(&rpm_parser);
}

static void __exit rpm_parser_exit(void)
{
	digest_cache_unregister_parser(&rpm_parser);
}

module_init(rpm_parser_init);
module_exit(rpm_parser_exit);

MODULE_AUTHOR("Roberto Sassu");
MODULE_DESCRIPTION("RPM digest list parser");
MODULE_LICENSE("GPL");
